...
owners = pd.read_json("data/league-element-status.json")["element_status"].apply(
    pd.Series
)

with open("data/league-details.json") as f:
    data = json.load(f)
    league_entries_df = pd.DataFrame(data["league_entries"]).dropna(axis=1, how="any")[
        ["entry_id", "entry_name", "short_name"]
    ]
league_entries_df

players_owners = pd.merge(
    owners, league_entries_df, left_on="owner", right_on="entry_id", how="left"
)

master_player_stats = pd.merge(
